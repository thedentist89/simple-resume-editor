import React from "react";
import { ReactComponent as Phone } from "../assets/phone.svg";
import { ReactComponent as Pin } from "../assets/pin.svg";
import { ReactComponent as Mail } from "../assets/mail.svg";
import { ReactComponent as SolidCircle } from "../assets/circle-solid.svg";
import { ReactComponent as Circle } from "../assets/circle.svg";

const Modern = () => {
  return (
    <div
      className="mx-auto bg-white flex py-6 shadow"
      style={{ height: "1262px", width: "892px" }}
    >
      <div className="w-2/6 mr-3">
        <div className="mx-auto w-40 py-6">
          <img
            src="data:image/jpeg;base64,"
            alt="Mourad Aouinat"
            className="rounded-full h-40 w-40"
          />
        </div>
        <div className="px-4">
          <div className="mt-4">
            <Mail className="w-5 h-5 fill-current text-purple-600" />
            <p className="text-sm">mourad.eyes@gmail.com</p>
          </div>
          <div className="mt-4">
            <Phone className="w-5 h-5 fill-current text-purple-600" />
            <p className="text-sm">+212690092625</p>
          </div>
          <div className="mt-4">
            <Pin className="w-5 h-5 fill-current text-purple-600" />
            <p className="text-sm">Kenitra, Morocco</p>
          </div>
          <div>
            <h3 className="text-purple-600 text-lg border-b-2 border-purple-600 font-semibold mt-4">
              SKILLS
            </h3>
            <div className="flex flex-wrap my-6">
              <span className="rounded bg-purple-600 text-white px-2 py-1 m-2">
                HTML
              </span>
              <span className="rounded bg-purple-600 text-white px-2 py-1 m-2">
                CSS
              </span>
              <span className="rounded bg-purple-600 text-white px-2 py-1 m-2">
                JavaScript
              </span>
              <span className="rounded bg-purple-600 text-white px-2 py-1 m-2">
                ReactJs
              </span>
              <span className="rounded bg-purple-600 text-white px-2 py-1 m-2">
                Sass
              </span>
              <span className="rounded bg-purple-600 text-white px-2 py-1 m-2">
                Web Design
              </span>
              <span className="rounded bg-purple-600 text-white px-2 py-1 m-2">
                Python
              </span>
              <span className="rounded bg-purple-600 text-white px-2 py-1 m-2">
                ES6
              </span>
            </div>
          </div>
          <div>
            <h3 className="text-purple-600 text-lg border-b-2 border-purple-600 font-semibold mt-4">
              LANGUAGES
            </h3>
            <div className="mt-6">
              <h3 className="mt-2">English</h3>
              <div className="flex mt-1">
                <SolidCircle className="w-6 h-6 fill-current text-purple-600 mr-2" />
                <SolidCircle className="w-6 h-6 fill-current text-purple-600 mr-2" />
                <SolidCircle className="w-6 h-6 fill-current text-purple-600 mr-2" />
                <SolidCircle className="w-6 h-6 fill-current text-purple-600 mr-2" />
                <Circle className="w-6 h-6 fill-current text-purple-600 mr-2" />
              </div>
            </div>
            <div>
              <h3 className="mt-2">French</h3>
              <div className="flex mt-1">
                <SolidCircle className="w-6 h-6 fill-current text-purple-600 mr-2" />
                <SolidCircle className="w-6 h-6 fill-current text-purple-600 mr-2" />
                <SolidCircle className="w-6 h-6 fill-current text-purple-600 mr-2" />
                <Circle className="w-6 h-6 fill-current text-purple-600 mr-2" />
                <Circle className="w-6 h-6 fill-current text-purple-600 mr-2" />
              </div>
            </div>
            <div>
              <h3 className="mt-2">Arabic</h3>
              <div className="flex mt-1">
                <SolidCircle className="w-6 h-6 fill-current text-purple-600 mr-2" />
                <SolidCircle className="w-6 h-6 fill-current text-purple-600 mr-2" />
                <SolidCircle className="w-6 h-6 fill-current text-purple-600 mr-2" />
                <SolidCircle className="w-6 h-6 fill-current text-purple-600 mr-2" />
                <SolidCircle className="w-6 h-6 fill-current text-purple-600 mr-2" />
              </div>
            </div>
          </div>
        </div>
      </div>
      <div className="w-4/6">
        <div className="p-4 bg-purple-600 text-white">
          <h1 className="text-4xl">Mourad Aouinat</h1>
          <h3 className="text-xl">Front-end Developer</h3>
          <div className="bg-white w-32 h-px"></div>
          <p className="mt-2">
            Meticulous web developer with over 2 years of front end experience
            and passion for responsive website design and a firm believer in the
            mobile-first approach. W3C certified. Implemented new responsive
            website approach which increased mobile traffic by 20%.
          </p>
        </div>
        <div className="pr-8 mt-6">
          <h3 className="text-purple-600 text-2xl border-b-2 border-purple-600 font-semibold mt-4">
            WORK EXPERIENCE
          </h3>
          <div className="py-10">
            <h4 className="text-2xl font-bold text-purple-600">
              Front-end Developer
            </h4>
            <h4 className="text-xl font-bold">Gemography</h4>
            <div className="flex justify-between text-purple-600">
              <p>12/2019 - 02/2020</p>
              <p>Rabat, Morocco</p>
            </div>
            <div className="mt-2">
              <p>Developed various web applications using ReactJS</p>
              <p>Converting Client Designs into functional Web applications</p>
              <p>Helped point out various UX/UI holes in designs</p>
            </div>
          </div>
        </div>
        <div className="pr-8 mt-6">
          <h3 className="text-purple-600 text-2xl border-b-2 border-purple-600 font-semibold mt-4">
            EDUCATION
          </h3>
          <div className="py-10">
            <div>
              <h4 className="text-2xl font-bold text-purple-600">
                Bachalor In Financial Management
              </h4>
              <h4 className="text-xl font-bold">
                Faculty of Economics and Business - Ibn Tofail
              </h4>
              <p className="text-purple-600">2018-2019</p>
            </div>
            <div className="mt-4">
              <h4 className="text-2xl font-bold text-purple-600">
                Bachaloriat In Economics and Management
              </h4>
              <h4 className="text-xl font-bold">Idriss Premier High School</h4>
              <p className="text-purple-600">2013-2014</p>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default Modern;
